...
class Blockchain {
    ...
    submitStar(address, message, signature, star) {
        return new Promise(async (resolve, reject) => {
            ...
            if (currentTime - messageTime > 300) reject("Time elapsed is more than 5 minutes");
            bitcoinMessage.verify(message, address, signature);
            const newBlock = new BlockClass.Block({ data:{ "star": star, "owner": address }})
    ...
    async getStarsByWalletAddress (address) {
        ...
        let stars = [];
        return new Promise((resolve) => {
            const filteredBlocks = self.chain.map(block => block.getBData());
            filteredBlocks.forEach(block => block && block.data.owner === address && stars.push(block.data));
            resolve(stars)
        }).catch(err => { throw err });
    }
    ...
    validateChain() {
        ...
        return new Promise(async (resolve) => {
            self.chain.forEach(async(block) => {
                ...
                if (!isValid) errorLog.push({ message: "Block is not valid", block: block.getBData() })
            })
            resolve(errorLog.length && errorLog)
        }).catch(err => { throw err });
    }
