...
class BlockchainController {
    ...
    // This endpoint allows you to check validation of blockchain
    getBlockchainValidation() {
        this.app.get("/validateBlockchain", async (req, res) => {
            try {
                let validationErrors = await this.blockchain.validateChain();
                if (validationErrors) {
                    return res.status(404).send("Blockchain is tampered!");
                } else {
